export const products = [
    {id: 1001,
    name:'Ножи для метания СМН+',
    descr: 'Сертифицированные ножи для спортсменов от 18 лет и старше, в комплекте 3 ножа, изготовлены из высоколегированного коррозионностойкого сплава, относящегося к мартенситному классу',
    price: 5600,
    img: [
        '/img/products/knifes/knife_smn_plus_001.png',
        '/img/products/knifes/knife_smn_plus_002.png'
    ],
    stock: true
    },
    {id: 1002,
    name:'Ножи для метания СМН',
    descr: 'Сертифицированные ножи для спортсменов младше 18 лет, в комплекте 3 ножа, изготовлены из высоколегированного коррозионностойкого сплава, относящегося к мартенситному классу',
    price: 5600,
    img: [
        '/img/products/knifes/knife_smn_001.png',
        '/img/products/knifes/knife_smn_002.png'
    ],
    stock: true
    },
    {id: 1003,
    name:'Гравировка на ножах',
    descr: 'Индивидуальная гравировка на ножах, стоимость гравировки зависит от сложности нанесения, размеров нанесения и количества макетов. Нанесение текста, логотипов и изображений',
    price: 2000,
    img: [
        '/img/products/knifes/engraving_002.png',
        '/img/products/knifes/engraving_001.png'
    ],
    stock: true
    },
    // {id: 1004,
    // name:'Трейд-ин ножей',
    // descr: 'К обмену принимаются только ножи СМН и СМН+, приобретенные в нашем магазине. Старый комплект из 3 ножей обменивается на новый комплект из 3 ножей',
    // price: 3600,
    // img: [
    //     '/img/products/knifes/trade_in_001.png'
    // ],
    // stock: true
    // },
    {id: 2001,
    name:'Чехол &laquo;ЦИФРА РФ&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_cifra_001.png',
        '/img/products/bags/case_cifra_002.png'
    ],
    stock: false
    },
    {id: 2002,
    name:'Чехол &laquo;МУЛЬТИКАМ&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_multicam_001.png',
        '/img/products/bags/case_multicam_002.png'
    ],
    stock: false
    },
    {id: 2003,
    name:'Чехол &laquo;ЧЕРНЫЙ&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_black_001.png',
        '/img/products/bags/case_black_002.png'
    ],
    stock: false
    },
    {id: 2004,
    name:'Чехол &laquo;АТАКС&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_ataks_001.png',
        '/img/products/bags/case_ataks_002.png'
    ],
    stock: false
    },    
    {id: 2005,
    name:'Чехол &laquo;ОРАНЖ&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_orange_001.png',
        '/img/products/bags/case_orange_002.png'
    ],
    stock: false
    },
    {id: 2006,
    name:'Чехол &laquo;ХАКИ&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_khaki_001.png',
        '/img/products/bags/case_khaki_002.png'
    ],
    stock: false
    },
    {id: 2007,
    name:'Чехол &laquo;АТАКС ГРИН&raquo; на 3 ножа',
    descr: 'Усиленные швы, с петлей для ремня, материал - ткань Оксфорд 600 D, максимальная вместимость до 4 ножей',
    price: 600,
    img: [
        '/img/products/bags/case_ataks_green_001.png',
        '/img/products/bags/case_ataks_green_002.png'
    ],
    stock: false
    },
    {id: 2008,
    name:'Кожаный чехол V1 на 3 ножа',
    descr: 'Материал - кожа, толщина кожи 3 мм, усиленные швы, застежка - кнопка, максимальная вместимость 3 ножа',
    price: 650,
    img: [
        '/img/products/bags/leather_case_001.png',
        '/img/products/bags/leather_case_002.png',
        '/img/products/bags/leather_case_003.png'
    ],
    stock: true
    },
    {id: 2009,
    name:'Кожаный чехол V2 на 3 ножа',
    descr: 'Материал - кожа, толщина кожи 3 мм, усиленные швы, застежка - кнопка, максимальная вместимость 3 ножа',
    price: 650,
    img: [
        '/img/products/bags/leather_case_v2_001.png',
        '/img/products/bags/leather_case_v2_002.png',
        '/img/products/bags/leather_case_v2_003.png'
    ],
    stock: false
    },
    {id: 3001,
    name:'Медальница &laquo;Движемся вперед&raquo;',
    descr: 'Материал - дерево, высота: 28 см, ширина: 43.5 см, крепежный набор в комплекте',
    price: 2500,
    img: [
        '/img/products/accessory/medallion_001.png',
        '/img/products/accessory/medallion_002.png',
        '/img/products/accessory/medallion_003.png'
    ],
    stock: true
    },
    {id: 3002,
    name:'Рамки для номеров',
    descr: 'Материал - пластик, рамки для государственных номеров, в комлекте 2 шт: 1 рамка с LED-подсветкой, 1 рамка с УФ-печатью',
    price: 3000,
    img: [
        '/img/products/accessory/frame_001.png',
        '/img/products/accessory/frame_002.png',
        '/img/products/accessory/frame_003.png'
    ],
    stock: true
    },
    {id: 3003,
    name:'Напильник с алмазной крошкой',
    descr: 'Миниатюрный напильник с алмазной крошкой (100/80 микрон), длина: 14 см, ширина: 2 см, толщина: 3 мм',
    price: 750,
    img: [
        '/img/products/accessory/file_001.jpg',
        '/img/products/accessory/file_002.jpg',
        '/img/products/accessory/file_003.jpg',
        '/img/products/accessory/file_004.jpg'
    ],
    stock: true
    },
    {id: 4001,
    name:'Стенд для метания 75',
    descr: 'Материал - дерево, размер 75х75х15 см, рабочая поверхность 60х60 см, наличие Сертификата Соответсвия',
    price: 7500,
    img: [
        '/img/products/equipment/stand.png',
        '/img/products/equipment/stand_02.png',
        '/img/products/equipment/stand_03.png'        
    ],
    stock: true
    },
    {id: 4002,
    name:'Стенд для метания 60',
    descr: 'Материал - дерево, размер 60х60х15 см, рабочая поверхность 45х45 см, наличие Сертификата Соответсвия',
    price: 5000,
    img: [
        '/img/products/equipment/stand_small_001.png'
    ],
    stock: true
    },
    {id: 4003,
    name:'Мишень для метания А3 (36х32см)',
    descr: 'Количество - 300 шт, материал - бумага, размер мишени 360х320 мм, плотность 200 г/м2, цветность 1+0', 
    price: 4500,
    img: [
        '/img/products/equipment/target_a3_001.png',
        '/img/products/equipment/target_a3_002.png'
    ],
    stock: true
    },
    {id: 4004,
    name:'Мишень для метания А4 (22х18см)',
    descr: 'Количество - 300 шт, материал - бумага, размер мишени 220х180 мм, плотность 200 г/м2, цветность 1+0', 
    price: 2850,
    img: [
        '/img/products/equipment/target_a4_001.png',
        '/img/products/equipment/target_a4_002.png'
    ],
    stock: true
    },

];
